...
export const documentCategorySchema = z.enum(['receipt', 'contract', 'id', 'warranty', 'medical', 'tax', 'other']);
...
export const documentSchema = z.object({
  ...
  fileUri: z.string().optional(), thumbnailUri: z.string().nullable().optional(),
  mimeType: z.string().min(1), fileSizeBytes: z.number().nonnegative(),
  pageCount: z.number().int().positive(),
  ocrText: z.string().optional(), ocrStatus: ocrStatusSchema,
  inferredDate: z.string().optional(), amounts: z.array(z.number()).optional(),
  vendor: z.string().optional(), isFavorite: z.boolean(),
  folderId: z.string().nullable(), tags: z.array(z.string()),
  createdAt: z.string(), updatedAt: z.string(),
  syncVersion: z.number().int().nonnegative().optional(),
});

export const folderSchema = z.object({
  id: z.string().min(1), name: z.string().min(1), color: z.string().min(1),
  createdAt: z.string(), updatedAt: z.string(),
  syncVersion: z.number().int().nonnegative().optional(),
});
...
export const shareLinkCreateSchema = z.object({
  documentId: z.string().min(1), title: z.string().min(1),
  expiresAt: z.string().datetime(),
  password: z.string().min(8).optional(),
});

export const emailInboundSchema = z.object({
  sender: z.string().email(), subject: z.string().default(''),
...
export const analyticsBatchSchema = z.object({
  events: z.array(z.object({
    ...
    deviceId: z.string().optional(), userId: z.string().optional(),
    properties: z.record(z.union([z.string(), z.number(), z.boolean()])).optional(),
  })).min(1).max(100),
});

export const userRegisterSchema = z.object({
  id: z.string().min(1), fullName: z.string().min(1),
  email: z.string().email(), passwordHash: z.string().min(1),
